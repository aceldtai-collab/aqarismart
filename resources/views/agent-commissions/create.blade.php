<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">{{ __('Record Commission') }}</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-md bg-white p-6 shadow">
                <form method="post" action="{{ route('agent-commissions.store') }}">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <select id="lease_id" name="lease_id" class="mt-1 block w-full rounded-md border-gray-300">
                            ...
                            @foreach(($leases ?? collect()) as $id => $label)
                                <option value="{{ $id }}" @selected(old('lease_id') == $id)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="rate" name="rate" type="number" step="0.01" class="mt-1 block w-full" value="{{ old('rate') }}" />
                    ...
                    <div class="mb-4">
                        ...
                        <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300" required>
                            @foreach(\App\Models\AgentCommission::statusLabels() as $value => $label)
                                <option value="{{ $value }}" @selected(old('status', \App\Models\AgentCommission::STATUS_PENDING) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>

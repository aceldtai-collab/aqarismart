...
<head>
    ...
    <title>{{ $isAr ? 'إنشاء إعلان جديد' : 'Create Listing' }} | {{ config('app.name') }}</title>
    <x-vite-assets />
    ...
    <main class="rl-shell" style="padding-top:8.5rem">
        <section class="rl-hero">
            <div class="rl-hero-copy">
                <div class="rl-kicker">{{ $isAr ? 'إعلان جديد' : 'New listing' }}</div>
                <div class="rl-ornament" style="margin-top:1rem"></div>
                <h1 style="margin:1.2rem 0 0;font-size:clamp(2rem,4vw,3.25rem);line-height:1.02;font-weight:900;letter-spacing:-.05em">{{ $isAr ? 'انشر عقارك مباشرة من السوق' : 'Post your property directly from the marketplace' }}</h1>
                <p style="margin:.9rem 0 0;max-width:56rem;font-size:1rem;line-height:1.85;color:rgba(255,255,255,.76)">{{ $isAr ? 'هذه هي النسخة الويب من تجربة إعلانات المالك. أضف التفاصيل والصور والمدة في مسار واحد واضح.' : 'This is the web version of the direct-owner listing journey. Add details, photos, and duration in one clear flow.' }}</p>
                <div style="display:flex;gap:.65rem;flex-wrap:wrap;margin-top:1.25rem">
                    <span class="rl-step-pill is-active" data-rl-step-pill="1">{{ $isAr ? '1. التفاصيل' : '1. Details' }}</span>
                    <span class="rl-step-pill" data-rl-step-pill="2">{{ $isAr ? '2. الصور' : '2. Photos' }}</span>
                    <span class="rl-step-pill" data-rl-step-pill="3">{{ $isAr ? '3. الدفع' : '3. Checkout' }}</span>
                </div>
            </div>
        </section>

        <form method="POST" action="{{ route('my-listings.store') }}" enctype="multipart/form-data" style="margin-top:1.25rem">
            @csrf
            @include('resident-listings.partials.form', [
                'wizard' => true,
                'submitLabel' => $isAr ? 'إنشاء الإعلان' : 'Create listing',
                'cancelUrl' => route('my-listings.index'),
            ])
        </form>
    </main>
</body>

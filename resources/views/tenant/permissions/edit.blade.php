    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-semibold text-xl text-gray-800">{{ __('Edit Role') }}: {{ $role->name }}</h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow rounded-lg">
                <form method="POST" action="{{ route('permissions.roles.edit', $role->id) }}">
                    @csrf
                    @method('PATCH')
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <h3 class="text-sm font-medium text-gray-900 mb-2">{{ __('Role Information') }}</h3>
                            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                <div>
                                    <dt class="text-gray-500">{{ __('Role Name') }}</dt>
                                    <dd class="font-medium text-gray-900">{{ $role->name }}</dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500">{{ __('Current Permissions') }}</dt>
                                    <dd class="font-medium text-gray-900">{{ count($rolePermissions) }}</dd>
                                </div>
                                <div>
                                ...
                                <div>
                                    <dt class="text-gray-500">{{ __('Users with this role') }}</dt>
                                    <dd class="font-medium text-gray-900">
                                        {{ $role->users()->wherePivot('tenant_id', $tenant->id)->count() }}
                                    </dd>
                                </div>
                            </dl>
                        </div>
                    ...
                    <div class="bg-gray-50 px-6 py-3 flex justify-between">
                        <div>
                            @if(!in_array($role->name, ['owner', 'admin', 'member']))
                                <form method="POST" action="{{ route('permissions.roles.delete', $role->id) }}" 
                                      onsubmit="return confirm('{{ __('Are you sure you want to delete this role?') }}')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 

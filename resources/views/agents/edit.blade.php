@php
    ...
    $updateAction = $tenantCtx ? route('agents.update', $agent) : route('admin.agents.update', $agent);
    $indexRoute = $tenantCtx ? route('agents.index') : route('admin.agents.index');
@endphp

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">{{ __('Edit Agent') }}</h2>
    </x-slot>
                ...
                <form method="post" action="{{ $updateAction }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="tenant_id" name="tenant_id" class="mt-1 block w-full rounded-md border-gray-300" required>
                            @foreach(\App\Models\Tenant::orderBy('name')->pluck('name','id') as $id => $name)
                                <option value="{{ $id }}" @selected($agent->tenant_id == $id)>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="name_en" name="name[en]" class="mt-1 block w-full" required :value="data_get($agent,'name_translations.en', $agent->getRawOriginal('name'))" />
                            <x-input-error :messages="$errors->get('name.en')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="name_ar" :value="__('Name (AR)')" />
                            <x-text-input id="name_ar" name="name[ar]" class="mt-1 block w-full" :value="data_get($agent,'name_translations.ar')" />
                            <x-input-error :messages="$errors->get('name.ar')" class="mt-2" />
                        </div>
                    </div>
                    <div class="mb-4">
                        <x-input-label for="email" :value="__('Email')" />
                            <x-text-input id="email" type="email" name="email" class="mt-1 block w-full" :value="$agent->email" />
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                    </div>
                    <div class="mb-6">
                        <x-input-label for="phone" :value="__('Phone')" />
                            <x-text-input id="phone" name="phone" class="mt-1 block w-full" :value="$agent->phone" />
                        <x-input-error :messages="$errors->get('phone')" class="mt-2" />
                    </div>
                    <div class="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <x-input-label for="license_id" :value="__('License ID')" />
                            <x-text-input id="license_id" name="license_id" class="mt-1 block w-full" :value="old('license_id', $agent->license_id)" />
                            <x-input-error :messages="$errors->get('license_id')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="commission_rate" :value="__('Commission Rate (%)')" />
                            <x-text-input id="commission_rate" name="commission_rate" type="number" step="0.01" min="0" max="100" class="mt-1 block w-full" :value="old('commission_rate', number_format((float) $agent->commission_rate, 2, '.', ''))" />
                    ...
                    <div class="mb-4">
                        ...
                        <p class="mt-1 text-xs text-gray-500">{{ __('Leave blank to keep the current logo.') }}</p>
                        @if($agent->photo_url)
                            <div class="mt-3 flex items-center gap-3">
                                <img src="{{ $agent->photo_url }}" alt="{{ $agent->name }}" class="h-12 w-12 rounded-full object-cover" />
                                <span class="text-sm text-gray-600">{{ __('Current logo') }}</span>
                            </div>
                        @endif
                    </div>
                    <div class="mb-6">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input id="active" type="checkbox" name="active" value="1" class="rounded border-gray-300 {{ $tenantCtx ? 'text-indigo-600 focus:ring-indigo-500' : 'text-[#e8604c] focus:ring-[#e8604c]' }}" @checked(old('active', $agent->active)) />

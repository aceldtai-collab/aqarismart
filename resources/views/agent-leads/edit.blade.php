<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">{{ __('Edit Lead') }}</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-md bg-white p-6 shadow">
                <form method="post" action="{{ route('agent-leads.update', $lead) }}">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <x-input-label for="name" :value="__('Name')" />
                        <x-text-input id="name" name="name" class="mt-1 block w-full" value="{{ old('name', $lead->name) }}" required />
                    ...
                    <div class="mb-4 grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="phone" name="phone" class="mt-1 block w-full" value="{{ old('phone', $lead->phone) }}" />
                    ...
                    <div class="mb-4 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="source" name="source" class="mt-1 block w-full" value="{{ old('source', $lead->source) }}" />
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300">
                                @foreach(\App\Models\AgentLead::statusLabels() as $value => $label)
                                    <option value="{{ $value }}" @selected(old('status', $lead->status ?? \App\Models\AgentLead::STATUS_NEW) === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="agent_id" name="agent_id" class="mt-1 block w-full rounded-md border-gray-300">
                                @foreach($agents as $id => $name)
                                    <option value="{{ $id }}" @selected(old('agent_id', $lead->agent_id) == $id)>{{ $name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full rounded-md border-gray-300">{{ old('notes', $lead->notes) }}</textarea>

<x-admin-layout>
    <x-slot name="header">{{ __('Edit Subcategory') }}</x-slot>
                ...
                <form method="post" action="{{ route('admin.subcategories.update', $subcategory) }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="name[en]" value="{{ old('name.en', data_get($subcategory,'name_translations.en', $subcategory->getRawOriginal('name'))) }}" class="gz-search w-full" required />
                        ...
                        <div>
                            ...
                            <input name="name[ar]" value="{{ old('name.ar', data_get($subcategory,'name_translations.ar')) }}" class="gz-search w-full" />
                    ...
                    <div>
                        ...
                        <input name="slug" value="{{ old('slug', $subcategory->slug) }}" class="gz-search w-full" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <textarea name="description[en]" rows="4" class="gz-search w-full">{{ old('description.en', data_get($subcategory,'description_translations.en', $subcategory->getRawOriginal('description'))) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="description[ar]" rows="4" class="gz-search w-full">{{ old('description.ar', data_get($subcategory,'description_translations.ar')) }}</textarea>
                            @error('description.ar')<div class="text-sm text-[#e8604c] mt-1">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-[#e8604c] focus:ring-[#e8604c]" {{ old('is_active', $subcategory->is_active) ? 'checked' : '' }} />
                        ...
                        <div class="ml-auto">
                            ...
                            <input type="number" name="sort_order" min="0" value="{{ old('sort_order', $subcategory->sort_order) }}" class="gz-search w-28" />
                        </div>
                    </div>

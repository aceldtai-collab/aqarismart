<x-app-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-slate-900">{{ __('Add Custom Attribute') }}</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form id="attribute-field-form" method="POST" action="{{ route('custom-attributes.store', request()->only('lang')) }}" class="bg-white rounded-xl border border-slate-200/60 shadow-sm p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="label_en" class="w-full border border-slate-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-slate-900 focus:border-slate-900" value="{{ old('label_en') }}" placeholder="{{ __('e.g. Swimming Pool') }}" required>
                    ...
                    <div>
                        ...
                        <input name="label_ar" class="w-full border border-slate-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-slate-900 focus:border-slate-900" value="{{ old('label_ar') }}" dir="rtl" placeholder="{{ __('e.g. مسبح') }}">
                ...
                <div>
                    ...
                    <input name="unit" class="w-full border border-slate-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-slate-900 focus:border-slate-900" value="{{ old('unit') }}" placeholder="{{ __('e.g. m², kg, years') }}">
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                    ...
                    <button type="submit" class="px-4 py-2.5 text-sm font-semibold text-white bg-slate-900 rounded-lg hover:bg-slate-800 transition-colors shadow-sm">{{ __('Create Attribute') }}</button>
                </div>
            </form>
        </div>
    </div>

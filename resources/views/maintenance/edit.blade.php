<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">{{ __('Edit Maintenance Request') }}</h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select id="unit_id" name="unit_id" class="mt-1 block w-full rounded-md border-gray-300">
                                ...
                                @foreach($units as $u)
                                    <option value="{{ $u->id }}" @selected(old('unit_id', $request->unit_id) == $u->id)>{{ $u->property?->name ?? __('Standalone') }} — {{ $u->code }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div>
                        <x-input-label for="resident_id" :value="__('Resident')" />
                        <select id="resident_id" name="resident_id" class="mt-1 block w-full rounded-md border-gray-300">
                            <option value="">{{ __('—') }}</option>
                            @foreach($residents as $r)
                                <option value="{{ $r->id }}" @selected(old('resident_id', $request->resident_id) == $r->id)>{{ $r->name }} ({{ $r->email }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <x-input-label for="title" :value="__('Title')" />
                        <x-text-input id="title" name="title" class="mt-1 block w-full" value="{{ old('title', $request->title) }}" required />
                    </div>
                    <div>
                        <x-input-label for="details" :value="__('Details')" />
                        <textarea id="details" name="details" class="mt-1 block w-full rounded-md border-gray-300" rows="4">{{ old('details', $request->details) }}</textarea>
                    </div>
                    <div>

...
function buildIcon(GdImage $logo): GdImage
{
    ...
    fillVerticalGradient($canvas, '#fbf6ed', '#eadcc0');
    drawGlow($canvas, 512, 380, 292, '#c39a53', 96, 127, 6);
    drawArcAccent($canvas, 512, 792, 612, 186, '#0f5a46', 112, 20, 160);
    drawArcAccent($canvas, 512, 812, 560, 150, '#b6842f', 118, 20, 160);

    $shadow = imagecolorallocatealpha($canvas, 22, 28, 23, 116);
    imagefilledellipse($canvas, 512, 754, 410, 70, $shadow);

    placeImageInBox($canvas, $logo, 118, 100, 788, 788);

    drawFlagPill($canvas, 318, 900, 388, 54, false);
...
function buildSplash(GdImage $logo, bool $dark): GdImage
{
    ...
    if ($dark) {
        fillVerticalGradient($canvas, '#0b1f19', '#17352b');
        drawGlow($canvas, 642, 920, 430, '#c7a15b', 100, 127, 8);
        drawRoundedPanel($canvas, 180, 420, 1104, 1498, 64, '#f3ebda', 0, '#ffffff', 78);
        imagefilledellipse($canvas, 642, 1540, 760, 122, imagecolorallocatealpha($canvas, 8, 18, 14, 112));
        drawArcAccent($canvas, 642, 2280, 780, 224, '#b6842f', 112, 16, 164);
        placeImageInBox($canvas, $logo, 228, 500, 828, 828);
        drawFlagPill($canvas, 382, 2290, 520, 68, true);
        drawFrame($canvas, $width, $height, '#27503f', 10, 95);
    } else {
        fillVerticalGradient($canvas, '#fbf4e7', '#e7d7b9');
        drawGlow($canvas, 642, 980, 460, '#caa760', 94, 127, 8);
        drawArcAccent($canvas, 642, 1660, 920, 360, '#0f5a46', 108, 18, 162);
        drawArcAccent($canvas, 642, 1690, 860, 288, '#b6842f', 114, 18, 162);
        imagefilledellipse($canvas, 642, 1618, 460, 72, imagecolorallocatealpha($canvas, 48, 60, 45, 118));
        placeImageInBox($canvas, $logo, 222, 520, 840, 840);
        drawFlagPill($canvas, 382, 2312, 520, 68, false);
...
function drawFlagPill(GdImage $image, int $x, int $y, int $width, int $height, bool $dark): void
{
    $shadowHex = $dark ? '#08140f' : '#1e231f';
    fillRoundedRect($image, $x + 12, $y + 14, $x + $width + 12, $y + $height + 14, (int) round($height / 2), $shadowHex, 112);

    $stripeHeight = (int) floor($height / 3);
    $radius = (int) round($height / 2);

    fillRoundedRect($image, $x, $y, $x + $width, $y + $height, $radius, '#f3ead8', $dark ? 16 : 0);
    ...
    $green = allocateHex($image, '#0f5a46', $dark ? 8 : 0);
    imagefilledellipse($image, $x + (int) round($width / 2), $y + (int) round($height / 2), 84, 18, $green);
}
